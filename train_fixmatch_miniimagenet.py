...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if args.dist_align and False: # no distribution alignment for baseline FixMatch
        ...
        print('High Confidence Ratio: {ratio:.2f}, Correct Pseudo-label Ratio: {ratio_pl:0.2f}'.format(
            ratio=100*(num_high_conf/num_total), ratio_pl=100*(pl_quality/num_total)
            ))
        print('Acc_high_conf: {:.2f}, Uncertainty of Discriminative classifier: {:.2f}'.format(
            100*(acc_high_conf/(num_high_conf+1e-7)), uncertainty_disc.avg)
            )
        if not args.no_progress:
            p_bar.close()
