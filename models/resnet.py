...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        #self.proj_head = nn.Linear(512 * block.expansion, 64)
    ...
    def forward(self, x, mixing=False, return_feat=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
def resnet34(num_classes=1000, pretrained=True):
