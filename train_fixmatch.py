...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            correct_high_conf = mask* correct_pl
            acc_high_conf += correct_high_conf.sum() # acc of high confidence pseudo-label
            num_high_conf += mask.sum()
            ...
            if not args.no_progress:
                ...
                if batch_idx % 100 == 0:
                    print("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Mask: {mask:.2f}.".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
        ...
        print('High Confidence Ratio: {ratio:.2f}, Correct Pseudo-label Ratio: {ratio_pl:0.2f}, Acc_high_conf: {ratio_co:0.2f}'.format(
            ratio=100*(num_high_conf/num_total), ratio_pl=100*(pl_quality/num_total),
            ratio_co=100*(acc_high_conf/(num_high_conf+1e-7))
            ))
        print('Uncertainty of Discriminative classifier: {:.2f}'.format(
            uncertainty_disc.avg
            ))
